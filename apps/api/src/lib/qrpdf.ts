...
export async function generateInvitationPDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(22).font('Helvetica-Bold').text('ETKİNLİK DAVETI', { align: 'center' })
    doc.moveDown()

    doc.fontSize(16).font('Helvetica').text(`Merhaba ${invitation.employee.name},`, { align: 'left' })
    doc.moveDown(0.5)
    ...
    doc.fontSize(14).font('Helvetica-Bold').text('Giriş QR Kodunuz', { align: 'center' })
    doc.moveDown(0.5)
    ...
    doc.fontSize(12).font('Helvetica').text('veya 9 haneli kodunuz:', { align: 'center' })
    doc.moveDown(0.3)
